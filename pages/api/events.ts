...
// ✅ SEGURANÇA: Configurações via variáveis de ambiente (CORRIGIDO)
const PIXEL_ID = process.env.META_PIXEL_ID || "1406446857128984";
const ACCESS_TOKEN = process.env.META_ACCESS_TOKEN || "EAALIy2dZAggsBPfaDSZBLNqUXZAG7fI1ovKKZBuFDDB1qtXdZAnyFYGlIkOZAKieaJcs1UzSqlp58MpHQMVR2QtJb4vmfARiyZBz0VGWpxtZCLTq3O7VZCAOWARWgm6Ddp8moSZAodNoBnNkv4ZBH5YDcQZBp2eLZCZCKjB7JeLtlgZCPUZA27ZA7KUtXiW7ZCdOq9sLHikQZDZD";
...
function hashSHA256(value: string): string {
  if (!value || typeof value !== "string") {
    console.warn("⚠️ hashSHA256: Valor inválido, usando fallback determinístico:", value);
    // Fallback determinístico baseado em timestamp e características do servidor
    const timestamp = Date.now();
    const processId = process.pid || 'unknown';
    const nodeVersion = process.version || 'unknown';
    const fallbackData = `fallback_${timestamp}_${processId}_${nodeVersion}_${typeof value}_${String(value).length}`;
...
// ✅ FORMATAÇÃO IPv6 OTIMIZADA: Para Meta CAPI
function formatIPForMeta(ip: string, ipType: string): string {
  if (ipType === "IPv6") {
    let cleanIP = ip.replace(/^\[|\]$/g, "");
    
    // ✅ OTIMIZAÇÃO: Não expandir IPv6 desnecessariamente
    // Meta CAPI aceita notação comprimida IPv6
    return cleanIP;
  }
  
  if (ipType === "IPv4") {
    // ✅ CONVERSÃO IPv4 para IPv6-mapped (recomendação Meta)
    // Formato: ::ffff:192.0.2.1
    return `::ffff:${ip}`;
  }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const headers = {
       ...
       "User-Agent": "TatySchapuis-CAPI-Proxy/1.0",
     ...
     if (error.name === "AbortError") {
       return res.status(408).json({ error: "Timeout ao enviar evento para a Meta", timeout_ms: 15000 });
     }
